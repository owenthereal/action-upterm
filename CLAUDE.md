...
- The countdown is driven by `firstGuestJoinedAt` alone (`hasGuestJoined()` in src/session.ts) - **never `guestCount`**: `guestCount` is `0` both when nobody has connected and when upterm's admin query failed (`hasLiveDetail === false`), so reading it as a live count risks stopping a session someone is attached to. Once a guest has ever joined, the countdown does not re-arm for the rest of the session.
