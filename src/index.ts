...
/**
 * `--join-timeout Nm` for an attached launch with wait-timeout-minutes above
 * 0; nothing otherwise. upterm counts it from readiness and ends the session
 * itself if no guest has joined by then; 0 means no timeout to upterm, and
 * here too. Detached mode never passes it: a window counted from launch would
 * end the session mid-build, so its window opens in the post step instead
 * (armJoinTimeout).
 */
function launchJoinTimeoutFlag(): string {
  if (core.getInput('detached') === 'true') return '';
  const minutes = parseInt(core.getInput('wait-timeout-minutes') || '0', 10);
  return minutes > 0 ? ` --join-timeout ${minutes}m` : '';
}
 ...
 *
 * 'unknown' deliberately ends nothing, so a lookup that fails on EVERY
 * iteration leaves a loop with no exit but the continue file. Warning makes
 * that visible to a human who can act on it; the deferral itself is not
 * negotiable, because ending the wait on an unknown would let teardown stop a
 * session a developer is attached to.
 */
...
/** Log the end of a session: unreachable, ended by upterm's join timeout, or quit. */
function logSessionEnded(session: SessionInfo): void {
  ...
  if (session.reason === 'join_timeout') {
    core.warning(`Timed out waiting for client to connect${session.joinTimeout ? ` (join timeout ${session.joinTimeout})` : ''}`);
...
/**
 * Ask upterm to end this run's session. Never throws: it is called from
 * teardown and from a signal handler - never from the wait, which only
 * observes - and in neither may upterm's own exit code, or a transient
...
/** wait-timeout-minutes in detached mode's post step: 10 minutes when unset or 0, as in v1. */
function detachedJoinTimeoutMinutes(): number {
  const minutes = parseInt(core.getInput('wait-timeout-minutes') || '10', 10);
  return isNaN(minutes) || minutes <= 0 ? 10 : minutes;
}

/**
 * Open detached mode's join window, now that the job's regular steps are done.
 * upterm owns the deadline from here; the action only reports what it said.
 *
 * Returns false only when upterm has no session by this name (exit 4): there
 * is nothing to wait for. Any other failure - in practice "could not confirm",
 * a daemon that did not answer in time - may or may not have taken effect, so
 * it is warned about and the wait runs anyway, bounded by the job's own
 * timeout. Not retried: a daemon that could not answer once is unlikely to
 * answer a retry, and the wait's log shows whether a deadline is counting.
 */
async function armJoinTimeout(): Promise<boolean> {
  const name = getSessionName();
  const minutes = detachedJoinTimeoutMinutes();
  try {
    const out = await execShellCommand(`upterm session set ${name} --join-timeout ${minutes}m`, {quiet: true});
    // upterm answers in one line that names the session ("session NAME ends
    // at…", "session NAME: …"); on Windows the login shell can print before
    // and after it, so pick it by name rather than by position.
    const answer = out
      .split(/\r?\n/)
      .map(line => line.trim())
      .find(line => line.startsWith(`session ${name} `) || line.startsWith(`session ${name}:`));
    if (answer) core.info(answer);
    return true;
  } catch (error) {
    if (isNoSuchSession(error)) {
      core.info("Exiting debugging session: 'upterm' quit");
    ...
    core.warning(`Could not confirm the ${minutes}-minute join timeout for upterm session ${name}; it may or may not have taken effect. Waiting anyway: the job's timeout-minutes bounds this wait. ${error}`);
    return true;
  }
}

type WaitEnd = 'continue' | 'ended';

/**
 * Wait for this run's session to end, or for the continue file. Observes only:
 * upterm alone ends a session nobody joined, at its own deadline, so no guest
 * can be kicked between a read and a stop, and nothing here stops a session.
 *
 * Logs the time left before upterm's joinDeadline and, once, the first join.
 * "Has a guest ever joined" is upterm's firstGuestJoinedAt, never guestCount,
 * and it latches: upterm disables the timeout for good at the first join, so
